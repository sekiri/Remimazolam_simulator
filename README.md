...
レミマゾラム（remimazolam）の **TCI（ターゲットコントロールドイフュージョン）** 推定シミュレータです。
静的投与（bolus）と持続投与（速度を途中で何度でも変更可）から、血中濃度 **Cp**、効果部位濃度 **Ce**、そして鎮静深度 **MOAA/S** の推移をリアルタイムに計算・可視化します。
...
- 検証：速度変更の増速/減速/終了・複数回変更・開始遅延・リセットなどを自動テスト（Puppeteer）で確認済み
