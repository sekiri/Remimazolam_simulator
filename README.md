...
投与プロトコル（静注 bolus の量・時刻、持続投与の開始時刻と速度、その後の速度変更）を入力すると、血中濃度 **Cp**、効果部位濃度 **Ce**、そして鎮静深度 **MOAA/S** の推移をリアルタイムに計算・可視化します。
...
- **静注（bolus）**：任意の時刻・量（mg）を複数回追加可能
- **持続投与（infusion）**：1 ポンプとして扱い、**ある時点での速度変更を何回でも追加**
  - 各変更 = その時刻から新速度で続行
  - **速度 0 = その時刻で投与終了**
- **mg / mg/h 入力の参考表示**：投与量・速度は臨床と同じ絶対量（mg、mg/h）で入力し、体重から mg/kg・mg/kg/h を自動計算して各欄の下方に表示（体重変更で自動再計算）
...
3. 投与プロトコルを設定
   - **静注**：`＋ 静注を追加` で時刻・量（mg）を追加。入力欄の下に mg/kg が参考表示されます
   - **持続投与**：開始時刻と初期速度（mg/h）を指定。速度を変えたいたびに `＋ 速度変更を追加` で `{変更時刻, 新速度}` を追加（速度 0 で終了）。同様に mg/kg/h が参考表示されます
4. **シミュレーションを実行** を押す
...
初期値は 60 歳・身長 165 cm・体重 65 kg・男性・ASA I・時間 120 分です。
投与イベントの欄は空欄で始まり、入力すると値が確定します（既定値はグレーのプレースホルダー表示）。
体重欄を変更すると、すべての mg/kg・mg/kg/h 参考表示が自動で再計算されます。
`初期化`（リセット）で患者情報・投与欄を既定値に戻して再計算します。

### 持続投与の速度変更例

| 変更時刻（分） | 新速度（mg/h） | 意味 |
|---|---|---|
| 0（開始） | 65 | 65 mg/h（65 kg なら ≒1.0 mg/kg/h）で開始 |
| 30 | 130 | 30 分時に 130 mg/h に増速 |
| 60 | 0 | 60 分時に投与終了 |
